...
func get_colors():
	return (_get_colors_from_gradient($GasLayers.material, "colorscheme") + _get_colors_from_gradient($Ring.material, "dark_colorscheme"))
	

func set_colors(colors):
	# poolcolorarray doesnt have slice function, convert to generic array first then back to poolcolorarray
	var cols1 = PackedColorArray(Array(colors).slice(0, 2, 1))
	var cols2 = PackedColorArray(Array(colors).slice(3, 5, 1))
